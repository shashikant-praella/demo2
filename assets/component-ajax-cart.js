...
class AjaxCart extends HTMLElement {
    constructor() {
      ...
      this.applyDiscountBtn = document.querySelector('[data-apply-discount]');
      if (this.applyDiscountBtn) this.applyDiscountBtn.addEventListener('click', this.applyDiscount.bind(this));
    }
 ...
 /*
  Free Shipping Bar Function
  we need to pass only cart json data as cartData parameter
  */
  async freeShippingBar() {
    let free_ship_limit = window.freeShipping.free_ship_limit;
    let free_ship_success = window.freeShipping.free_ship_success;
    let free_ship_away = window.freeShipping.free_ship_away;
    let cartData = await this.cartJSON();
    let eligibleLimit = free_ship_limit*100;

    if(document.getElementById('free-shipping-bar') && eligibleLimit > 0)
    {
      let progressBarWidth = 100;
      let barHtml = '<div class="delivery-shipping d-flex mb-3"><a href="#" class="me-2 lh-1"><span class="icon-free-shipping"></a>';
      if(cartData.total_price > eligibleLimit)
      {
        barHtml = barHtml + '<p class="mb-0">'+free_ship_success+'</p>';
      }
      else
      {
        progressBarWidth = (cartData.total_price/free_ship_limit).toFixed(5);
        let leftAmount = eligibleLimit-cartData.total_price;
        leftAmount = Shopify.formatMoney(leftAmount, window.globalVariables.money_format);
        free_ship_away = free_ship_away.replace('${addMoreForFreeShip}',leftAmount);
        barHtml = barHtml + '<p class="mb-0">'+free_ship_away+'</p>';
      }
      barHtml = barHtml + '</div><div class="progress m-3"><div class="progress-bar d-block" style="width:'+progressBarWidth+'%;" role="progressbar"></div></div>';
      document.getElementById('free-shipping-bar').innerHTML = barHtml;
    }//if-closed
  }
     /*
    ...
    */
    async applyDiscount(event){
      let discountCode = this.querySelector('[data-discountCode]').value;
      let cartJSON = await this.cartJSON();
      const cartItems = cartJSON.items;
      let linItemsArray = []
      cartItems.forEach((item,i) => {
        let itemID = item.id.toString();
        linItemsArray.push({"variant_id":itemID,"quantity":item.quantity});
      });
      console.log(JSON.stringify(linItemsArray))
      fetch("/payments/config", {
          }).then(response => {
              return response.json();
          }).then(response => {
            console.log('response',response.paymentInstruments.accessToken);
            fetch("/wallets/checkouts/?locale=en", {
            "credentials": "include",
            "headers": {
                "Content-Type": "application/json",
                "authorization": "Basic " + btoa(response.paymentInstruments.accessToken),
                "cache-control": "no-cache",
                "pragma": "no-cache",
                "Alt-Used": Shopify.shop,
            },
            "referrer": "https://"+Shopify.shop+"/",
            "body": "{\"checkout\":{\"discount_code\":\""+discountCode+"\",\"line_items\":"+JSON.stringify(linItemsArray)+",\"presentment_currency\":\"USD\"}}",
            "method": "POST"              
            }).then(response => {
                return response.json();
            }).then(response => {
    ...
    updateCartData(checkoutHTML){
      console.log('response',checkoutHTML.querySelector('[data-order-summary-section="payment-lines"]'));
      let orderHtml = checkoutHTML.querySelector('[data-order-summary-section="line-items"]');
      let _this = this;
      checkoutHTML.querySelectorAll('.product').forEach(element => {
        let UpdatedPrice = element.querySelector('.product__price span').innerHTML;
        let variantID = element.getAttribute('data-variant-id');
        let variantDiv = _this.querySelector('[data-variant-id="'+variantID+'"]');
        variantDiv.querySelector('.cart-total-price span').innerHTML = UpdatedPrice ;
      });
      this.querySelector('[data-checkoutbtns]').innerHTML = checkoutHTML.querySelector('[data-order-summary-section="payment-lines"]').innerHTML;
    }
}
